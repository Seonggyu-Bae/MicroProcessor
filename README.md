...
### Code Memory(program [0:1023])를 읽어와서 tCPU 내부에 있는 Decode unit 을 사용해 프로그램을 해석하고 Execute Unit 을 통해 실행하는 기본적인? 16-bit CPU의 구조를 가지고 있다. 연산은 register[0:15] 에서 이루어지며 register 의 저장공간이 모자랄 경우 memory[0:255]에 저장을 해놓고 다시 가져다 사용하는 방식이다. 
